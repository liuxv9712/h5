(function($) {
	...
	$.TopBar = function(el, option) {
		var canvas = $(el);
		canvas.settings = $.extend({}, $.TopBar.default, option);
		// 设置样式
		var funs = {};
		funs = {
			initChart: function() {
				...
				canvas.settings.chart.coord({
					transposed: true
				});
				canvas.settings.chart.axis('xm', {
					line: F2.Global._defaultAxis.line,
					grid: null
				});
				canvas.settings.chart.axis('bzxcount', {
					line: null,
				...
				canvas.settings.chart.interval().position('xm*bzxcount').color('#FF8800').size(15);
		...
		this.changeData = function(data) {
			if(!$.isEmptyObject(data)){
				...
				canvas.settings.chart.changeData(data);
			}else{
				this.showEmpty();
			}
			return;
