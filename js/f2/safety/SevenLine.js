(function($) {
	...
	$.SevenLine = function(el, option) {
		var canvas = $(el);
		canvas.settings = $.extend({}, $.SevenLine.default, option);
		// 设置样式
		var funs = {};
		funs = {
			initChart: function() {
				...
				canvas.settings.chart.axis('name', {
				...
				canvas.settings.chart.line().position('name*value');
				canvas.settings.chart.point().position('name*value').style({
					stroke: '#fff',
					lineWidth: 1
				});
